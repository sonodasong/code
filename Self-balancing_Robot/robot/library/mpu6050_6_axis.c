...
const uint8 dmpMemory[MPU6050_DMP_CODE_SIZE] PROGMEM = {
	// bank 0, 256 bytes
...
const uint8 dmpConfig[MPU6050_DMP_CONFIG_SIZE] PROGMEM = {
...
const uint8 dmpUpdates[MPU6050_DMP_UPDATES_SIZE] PROGMEM = {
...
uint8 MPU6050dmpInitialize(void)
{
	...
	xgOffsetTC = MPU6050getXGyroOffsetTC();
	ygOffsetTC = MPU6050getYGyroOffsetTC();
	zgOffsetTC = MPU6050getZGyroOffsetTC();
	...
	if (MPU6050writeProgMemoryBlock(dmpMemory, MPU6050_DMP_CODE_SIZE, 0, 0, TRUE)) {
		if (MPU6050writeProgDMPConfigurationSet(dmpConfig, MPU6050_DMP_CONFIG_SIZE)) {
			...
			MPU6050setXGyroOffsetTC(xgOffsetTC);
			MPU6050setYGyroOffsetTC(ygOffsetTC);
			MPU6050setZGyroOffsetTC(zgOffsetTC);
			pos = 0;
...
uint8 mpu6050DMPInitFromTask(void)
{
	...
	MPU6050initialize();
	devStatus = MPU6050dmpInitialize();
	MPU6050setXGyroOffset(61);
	MPU6050setYGyroOffset(62);
	MPU6050setZGyroOffset(0);
	MPU6050setZAccelOffset(1688);
	if (devStatus == 0) {
		MPU6050setDMPEnabled(TRUE);
		mpu6050IRQClear();
		mpu6050IRQEnable();
